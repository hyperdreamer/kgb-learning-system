...
- **Review**: The front shows the sentence with target surface spans in bold. The back shows the same highlighted sentence plus each expression with its contextual meaning once (no bullet list; the derived `cards.back` cache is not re-appended).
